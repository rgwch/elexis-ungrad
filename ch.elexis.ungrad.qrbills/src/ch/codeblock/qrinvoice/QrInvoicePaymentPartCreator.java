/*-
 * #%L
 * QR Invoice Solutions
 * %%
 * Copyright (C) 2017 - 2022 Codeblock GmbH
 * %%
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 * -
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 * -
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 * -
 * Other licenses:
 * -----------------------------------------------------------------------------
 * Commercial licenses are available for this software. These replace the above
 * AGPLv3 terms and offer support, maintenance and allow the use in commercial /
 * proprietary products.
 * -
 * More information on commercial licenses are available at the following page:
 * https://www.qr-invoice.ch/licenses/
 * #L%
 */
package ch.codeblock.qrinvoice;

import ch.codeblock.qrinvoice.model.QrInvoice;
import ch.codeblock.qrinvoice.output.PaymentPartReceipt;

import java.util.Locale;

/**
 * @deprecated since version 1.0, use {@link QrInvoicePaymentPartReceiptCreator} instead
 */
@Deprecated
public class QrInvoicePaymentPartCreator extends QrInvoicePaymentPartReceiptCreator {

    private QrInvoicePaymentPartCreator() {
    }

    public static QrInvoicePaymentPartCreator create() {
        return new QrInvoicePaymentPartCreator();
    }

    @Override
    public QrInvoicePaymentPartCreator qrInvoice(final QrInvoice qrInvoice) {
        super.qrInvoice(qrInvoice);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator outputFormat(final OutputFormat outputFormat) {
        super.outputFormat(outputFormat);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator outputResolution(final OutputResolution outputResolution) {
        super.outputResolution(outputResolution);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator pageSize(final PageSize pageSize) {
        super.pageSize(pageSize);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator fontFamily(final FontFamily fontFamily) {
        super.fontFamily(fontFamily);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator locale(final Locale locale) {
        super.locale(locale);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator boundaryLines(final boolean boundaryLines) {
        super.boundaryLines(boundaryLines);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator withBoundaryLines() {
        return boundaryLines(true);
    }

    @Override
    public QrInvoicePaymentPartCreator withoutBoundaryLines() {
        return boundaryLines(false);
    }

    @Override
    public QrInvoicePaymentPartCreator withScissors() {
        return boundaryLineScissors(true);
    }

    @Override
    public QrInvoicePaymentPartCreator boundaryLineScissors(final boolean boundaryLineScissors) {
        super.boundaryLineScissors(boundaryLineScissors);
        return this;
    }

    @Override
    public QrInvoicePaymentPartReceiptCreator withoutScissors() {
        return boundaryLineScissors(false);
    }

    @Override
    public QrInvoicePaymentPartCreator withSeparationText() {
        return boundaryLineSeparationText(true);
    }

    @Override
    public QrInvoicePaymentPartReceiptCreator withoutSeparationText() {
        return boundaryLineSeparationText(false);
    }

    @Override
    public QrInvoicePaymentPartCreator boundaryLineSeparationText(final boolean boundaryLineSeparationText) {
        super.boundaryLineSeparationText(boundaryLineSeparationText);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator inGerman() {
        this.locale(Locale.GERMAN);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator inFrench() {
        this.locale(Locale.FRENCH);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator inEnglish() {
        this.locale(Locale.ENGLISH);
        return this;
    }

    @Override
    public QrInvoicePaymentPartCreator inItalian() {
        this.locale(Locale.ITALIAN);
        return this;
    }

    public PaymentPartReceipt createPaymentPart() {
        return super.createPaymentPartReceipt();
    }

}
