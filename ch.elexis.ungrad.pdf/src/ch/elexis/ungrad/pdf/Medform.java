  ...
  public String create(String outPath, Patient pat) throws Exception {
    ...
    if (kons != null) {
      ...
      if (fall != null) {
        ...
        if (cb != null) {
          m.put(get("insuranceName"), cb.get("Bezeichnung1") + " " + cb.get("Bezeichnung2"));
          m.put(get("insuranceStreet"), StringTool.unNull(cb.get("Strasse")));
          m.put(get("insuranceZip"), StringTool.unNull(cb.get("Plz")));
          m.put(get("insuranceCity"), StringTool.unNull(cb.get("Ort")));
          m.put(get("insuranceMail"), StringTool.unNull(cb.get("E-Mail")));

        }
        BillingLaw bl = fall.getConfiguredBillingSystemLaw();
        String nr = fall.get(Fall.FLD_VERS_NUMMER);
        if (StringTool.isNothing(nr)) {
          nr = fall.get(Fall.FLD_FALL_NUMMER);
        }
        if (StringTool.isNothing(nr)) {
