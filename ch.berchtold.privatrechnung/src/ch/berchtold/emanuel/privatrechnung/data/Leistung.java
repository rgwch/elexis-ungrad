...
public class Leistung extends VerrechenbarAdapter {
	...
	private static final String createDB =
		"CREATE TABLE " + TABLENAME + "(" + "ID				VARCHAR(25) primary key,"// This field must always be present
			+ "deleted		CHAR(1) default '0'," + "lastupdate	BIGINT,"// This field must always be present
			+ "parent			VARCHAR(80)," + "name			VARCHAR(499),"
			+ "short			VARCHAR(80)," + "cost			CHAR(8),"// use always fixed char fields for amounts
			+ "price			CHAR(8),"// amounts are always in cents/rp
			+ "time			CHAR(4)," + "subsystem		VARCHAR(25)," + "valid_from		CHAR(8),"// use always char(8) for dates
			+ "valid_until	CHAR(8)," + "ExtInfo		BLOB);" + // An ExtInfo field can be used to store arbitrary data
