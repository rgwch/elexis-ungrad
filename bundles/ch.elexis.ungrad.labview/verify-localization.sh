#!/bin/bash
# Localization Configuration Verification Script for ch.elexis.ungrad.labview

echo "╔════════════════════════════════════════════════════════════════════╗"
echo "║   Labview Localization Configuration Verification                 ║"
echo "╚════════════════════════════════════════════════════════════════════╝"
echo ""

BUNDLE_DIR="/home/gerry/git/elexis-ungrad-plugins/bundles/ch.elexis.ungrad.labview"
cd "$BUNDLE_DIR"
...
if [ -f "src/ch/elexis/ungrad/labview/Messages.java" ]; then
...
for lang in "" "_de" "_en" "_fr" "_it"; do
    file="src/ch/elexis/ungrad/labview/messages${lang}.properties"
    if [ -f "$file" ]; then
...
java_files=(
    "views/LaborView.java"
    "views/PreferencePage.java"
)

for file in "${java_files[@]}"; do
    path="src/ch/elexis/ungrad/labview/$file"
    if grep -q "ch.elexis.ungrad.labview.Messages\." "$path" 2>/dev/null; then
