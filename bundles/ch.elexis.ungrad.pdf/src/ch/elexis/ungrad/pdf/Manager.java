 ...
 * Uses https://github.com/danfickle/openhtmltopdf and
...
public class Manager {
	 ...
	 * @param fields     fields to write. fields in the form not mentioned herein
	 *                   remain simply unchanged. Fields not found in the form are
	 *                   ignored.
	...
	public String getFieldContents(String formPath, String fieldName) throws Exception {
		PDDocument pdfDoc = PDDocument.load(new File(formPath));
		PDDocumentCatalog docCatalog = pdfDoc.getDocumentCatalog();
		PDAcroForm acroForm = docCatalog.getAcroForm();
	 ...
	 * 
	 * @param inputHtml  the file to read. If a style sheet is linked, it must be in
	 *                   the current dir or an absolute path.
	 ...
	 * @param printer The printer to use. If it is null or the named printer is not
	 *                found, a printer select dialog will open.
