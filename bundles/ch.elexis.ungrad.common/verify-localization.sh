#!/bin/bash
# Localization Configuration Verification Script for ch.elexis.ungrad.common

echo "╔════════════════════════════════════════════════════════════════════╗"
echo "║   Common Localization Configuration Verification                  ║"
echo "╚════════════════════════════════════════════════════════════════════╝"
echo ""

BUNDLE_DIR="/home/gerry/git/elexis-ungrad-plugins/bundles/ch.elexis.ungrad.common"
cd "$BUNDLE_DIR"
...
if [ -f "src/ch/elexis/ungrad/common/Messages.java" ]; then
...
for lang in "" "_de" "_en" "_fr" "_it"; do
    file="src/ch/elexis/ungrad/common/messages${lang}.properties"
    if [ -f "$file" ]; then
...
java_files=(
    "ui/Preferences.java"
    "ui/ExtIdPreferences.java"
)

for file in "${java_files[@]}"; do
    path="src/ch/elexis/ungrad/common/$file"
    if grep -q "ch.elexis.ungrad.common.Messages\." "$path" 2>/dev/null; then
