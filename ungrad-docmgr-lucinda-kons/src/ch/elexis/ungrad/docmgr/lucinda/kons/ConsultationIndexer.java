...
public class ConsultationIndexer implements Customer {
	...
	/**
	 * If active, the Index will run over all Consultations. If bActive==false, Indexing will stop
	...
	/**
	 * Start indexing. All consultations since last run are fetched from the database. A progress
	 * indicator and a Sender are initialized. Note: Possibly, there exist more than
	 * Integer.MAX_VALUE consultations. So the List.size() call would not be correct.
	 * 
	 * @throws IOException
	 * 
	 * @See Sender
	...
	public void start(IProgressController pc){
		...
		Collection<Konsultation> konsen =
			qbe.queryExpression(querySQL.toString(), new LinkedList<Konsultation>());
	...
	/**
	 * for each hit in the List, the Sender asks its Customer to fill in values to store in the
	 * index. Mandatory fields are as follows:
	 * <ul>
	 * <li>title: A short text describing the entry. Will show up in search results</li>
	 * <li>type: A description (one word) of the type of this entry. Well also show up in the
	 * results</li>
	 ...
	 * <li>concern: a standardized description of the patient: lastname_firstname_birthdate. this
	 * will be used to filter entries of the currently selected patient in searches.</li>
	 * </ul>
	 * 
	 * Other fields are optional.
	 * 
	 * @Returns a Map with the metadata, or null to indicate,that the sender should finish and
	...
	public Map<String, Object> specify(PersistentObject po){
		...
		if (cont) {
			...
			String konsdate = new TimeTool(kons.getDatum()).toString(TimeTool.DATE_ISO);
			StringBuilder concern =
				new StringBuilder().append(lastname).append("_").append(firstname).append("_") //$NON-NLS-1$ //$NON-NLS-2$
					.append(birthdate.substring(6)).append(".").append(birthdate.substring(4, 6)) //$NON-NLS-1$
					.append(".") //$NON-NLS-1$
	...
	/**
	 * When all elements are processed, or after the customer answered "null" to the call to
	 * specify, the Sender calls finished for cleanup. Here we note the date of the last
	 * consultation indexed to continue later.
	 *
	 * @param messages
